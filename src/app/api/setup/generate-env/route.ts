...
export async function POST(req: Request) {
  try {
    ...
    if (format === 'example') {
      ...
      envContent += '# Ejemplo: mongodb+srv://usuario:contraseña@cluster.mongodb.net/Restaurant\n';
      envContent += `MONGODB_URI=${mongodb?.uri || ''}\n\n`;
      envContent += '# Configuración de Email SMTP (Gmail)\n';
      envContent += '# Usar contraseña de aplicación de Google, no la contraseña normal\n';
...
function generateSecret(): string {
  ...
  return result;
    ...
    return NextResponse.json({ error: error.message || 'Error generando .env.local' }, { status: 500 });
  }
}
