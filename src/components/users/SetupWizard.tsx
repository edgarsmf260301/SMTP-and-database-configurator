...
export default function SetupWizard() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      const remaining = Math.max(
        0,
        Math.floor((tokenExpires.getTime() - Date.now()) / 1000)
      );
  ...
  const handleMongoDBSubmit = async (data: { uri: string }) => {
    ...
    try {
      ...
      setSetupData(prev => {
        const updated = { ...prev, mongodb: data };
        try { localStorage.setItem('setupData', JSON.stringify(updated)); } catch {}
        return updated;
      });
      goToNextStep('smtp');
    } catch (err: unknown) {
  ...
  const handleSMTPSubmit = async (data: {
    email: string;
    password: string;
  }) => {
    setIsLoading(true);
  ...
  const handleAdminSubmit = async (data: {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || 'Error al crear usuario administrador'
        );
      }
    ...
    if (currentStep === 'checking') {
      ...
      return (
        ...
        >
          ...
          >
            Por favor espera unos segundos mientras verificamos tu archivo de
    ...
    switch (currentStep) {
        ...
        return (
          ...
          >
            ...
            <motion.p
              ...
              variants={itemVariants}
            >
              Vamos a configurar tu sistema paso a paso. Este proceso te guiará
              para establecer la conexión a la base de datos, configurar el
              envío de emails y crear tu cuenta de administrador.
            </motion.p>
            <motion.button
          ...
          >
            ...
            >
              ...
              <p className="text-gray-300 text-xs sm:text-sm mb-2">
                Para permitir todas las conexiones, configura tu IP como{' '}
                <code className="bg-gray-600 px-1 sm:px-2 py-1 rounded text-yellow-300 text-xs">
                  0.0.0.0/0
                </code>
              </p>
              <p className="text-gray-300 text-xs sm:text-sm">
                Si tienes conocimientos avanzados y un proveedor con IP fija,
                configura esa IP específica para mayor seguridad.
              </p>
            </motion.div>
              ...
              <div>
                ...
                {mongodbForm.formState.errors.uri && (
                  <p className="text-red-400 text-xs sm:text-sm mt-1">
                    {mongodbForm.formState.errors.uri.message}
                  </p>
                )}
              </div>
          ...
          <div className="max-w-2xl mx-auto space-y-4 sm:space-y-6">
            <div className="text-center">
              ...
              <p className="text-sm sm:text-base text-gray-300">
                Configura las credenciales de Gmail SMTP
              </p>
            </div>
            ...
            >
              <div>
                ...
                {smtpForm.formState.errors.email && (
                  <p className="text-red-400 text-xs sm:text-sm mt-1">
                    {smtpForm.formState.errors.email.message}
                  </p>
                )}
              </div>
      ...
      case 'admin':
        ...
        const canResend =
          timeLeft === 0 &&
          (resendAttempts < 1 ||
            (resendCooldown && new Date() > resendCooldown));
        const cooldownLeft = resendCooldown
          ? Math.max(
              0,
              Math.floor((resendCooldown.getTime() - Date.now()) / 1000)
            )
          : 0;
          ...
          return (
            ...
            >
              ...
              <motion.p
                ...
                transition={{ duration: 0.6, delay: 0.4 }}
              >
                Por favor espera unos segundos mientras verificamos la base de
                datos.
              </motion.p>
            </motion.div>
          );
        ...
        if (adminExists === 'exists') {
          const handleCompleteWithEnv = async () => {
            setIsLoading(true);
            setError('');
            let mongodb = setupData.mongodb;
            let smtp = setupData.smtp;
            // Si los datos están vacíos, intenta recuperarlos de localStorage
            if (!mongodb?.uri || !smtp?.email || !smtp?.password) {
              try {
                const saved = localStorage.getItem('setupData');
                if (saved) {
                  const parsed = JSON.parse(saved);
                  mongodb = parsed.mongodb || mongodb;
                  smtp = parsed.smtp || smtp;
                }
              } catch {}
            }
            // Validar datos antes de enviar
            if (!mongodb?.uri || !smtp?.email || !smtp?.password) {
              setError('Debes completar la configuración de MongoDB y SMTP antes de continuar.');
              setIsLoading(false);
              return;
            }
            try {
              ...
              if (!response.ok) {
                const errorData = await response.json();
                throw new Error(
                  errorData.error || 'Error al generar archivo de entorno'
                );
              }
              goToNextStep('complete');
            } catch (err: unknown) {
          ...
          return (
            ...
            >
              ...
              <motion.p
                ...
                transition={{ duration: 0.6, delay: 0.4 }}
              >
                Puedes continuar con la configuración o iniciar sesión con tu
              ...
              >
                ...
                >
                  ...
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth="2.5"
                    d="M12 17c-4 0-7 2-7 4v1a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-1c0-2-3-4-7-4z"
                  />
                </svg>
              </motion.div>
              <motion.h2
          ...
          return (
            ...
            >
              ...
              <motion.p
                ...
                transition={{ duration: 0.6, delay: 0.4 }}
              >
                Se ha enviado un token de verificación a{' '}
                <span className="text-blue-400 font-medium">
                  {setupData.admin.email}
                </span>
                . Revisa tu bandeja de entrada y carpeta de spam. El token
                expira en{' '}
      ...
      case 'complete':
        // Limpiar tokens de autenticación y localStorage para evitar problemas de JWT inválidos
        useEffect(() => {
          try {
            // Elimina cookies de autenticación (NextAuth, JWT, etc.)
            document.cookie.split(';').forEach(cookie => {
              const eqPos = cookie.indexOf('=');
              const name = eqPos > -1 ? cookie.substr(0, eqPos) : cookie;
              document.cookie = name + '=;expires=Thu, 01 Jan 1970 00:00:00 GMT;path=/';
            });
            // Limpia localStorage de posibles tokens
        ...
        return (
          <div className="text-center space-y-4 sm:space-y-6">
            ...
            <p className="text-sm sm:text-lg text-gray-300 max-w-2xl mx-auto px-4">
              Tu sistema de restaurante ha sido configurado exitosamente. Ahora
              puedes iniciar sesión con tu cuenta de administrador para comenzar
              a usar todas las funcionalidades.
            </p>
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center">
        ...
        >
          ...
          <motion.ul className="mt-6 space-y-2 text-left">
            <motion.li className="text-blue-400 text-sm font-medium">
              Verificando archivos de configuración
            </motion.li>
            <motion.li className="text-blue-400 text-sm font-medium">
              Comprobando conexión a base de datos
            </motion.li>
            <motion.li className="text-gray-400 text-sm font-medium">
              Validando usuario administrador
            </motion.li>
          </motion.ul>
        </motion.div>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-3 sm:mb-4 transition-all duration-500 ease-out">
                    <p
                      className={`text-xs sm:text-sm lg:text-base font-semibold text-center transition-all duration-300 ${
                        isActive
                  ...
                  >
                    ...
                    <div className="relative z-10 transition-all duration-300">
                      {isCompleted ? (
                        <svg
                          className={`w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 text-white transform scale-100 transition-all duration-500 ${
